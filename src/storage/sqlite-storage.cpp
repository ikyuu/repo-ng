...
SqliteStorage::initializeRepo()
{
  ...
  if (rc == SQLITE_OK) {
    sqlite3_exec(m_db, "CREATE TABLE NDN_REPO (name BLOB, data BLOB);", nullptr, nullptr, &errMsg);
    // Ignore errors (when database already exists, errors are expected)
    sqlite3_exec(m_db, "CREATE UNIQUE INDEX index_name ON NDN_REPO (name);", nullptr, nullptr, &errMsg);
  }
  else {
...
SqliteStorage::insert(const Data& data)
{
  ...
  ndn::util::Sqlite3Statement stmt(m_db, "INSERT INTO NDN_REPO (name, data) VALUES (?, ?);");

  //Insert
  // Bind NULL to name value in NDN_REPO when initialize result.
...
{
  ndn::util::Sqlite3Statement stmt(m_db, "DELETE FROM NDN_REPO WHERE name = ?;");
...
SqliteStorage::size()
{
  ndn::util::Sqlite3Statement stmt(m_db, "SELECT count(*) FROM NDN_REPO;");
